...
const Navbar: FC = () => {
  ...
  const getLinkClass = ({ isActive }: { isActive: boolean }): string =>
    `transition-opacity hover:opacity-60 ${isActive ? "underline underline-offset-8 decoration-1" : ""}`;
  ...
  return (
    <div className="flex items-center gap-10">
      ...
      <nav className="hidden md:flex items-center gap-8 text-[11px] font-semibold uppercase tracking-widest">
        ...
        <div className="relative group cursor-pointer">
          ...
          <div className="absolute left-0 top-full pt-4 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-60">
            <div className="
              min-w-200px 
              rounded-sm py-3 shadow-2xl border
              /* MODO CLARO */
            ...
            ">
              {categories.map((cat) => (
                <NavLink
                  ...
                  to={cat.path}
                  className="
                    block px-6 py-2 text-[10px] transition-colors
                    /* Hovers dinámicos */
      ...
      {mobileOpen && (
        <div className="
          absolute top-full left-0 w-full flex flex-col p-8 gap-6 md:hidden z-50
          bg-white text-black border-b border-neutral-200
          dark:bg-black dark:text-white dark:border-neutral-800
        ">
          <NavLink to="/" onClick={() => setMobileOpen(false)} className="text-sm tracking-widest uppercase">
            Home
          </NavLink>
          <div className="h-1px bg-neutral-100 dark:bg-neutral-800 w-full" />
          {categories.map((cat) => (
            <NavLink 
              key={cat.name} 
              to={cat.path} 
              onClick={() => setMobileOpen(false)}
              className="text-sm tracking-widest uppercase opacity-80 hover:opacity-100"
            >
              {cat.name}
            </NavLink>
          ))}
